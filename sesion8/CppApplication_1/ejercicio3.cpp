...
/* 
 * File:   ejercicio3.cpp
 * Author: Mario Antonio López Ruiz ~ 45109755q
 *
 * Created on 28 de diciembre de 2016, 18:11
 */
...
int main(){
	...
	char entrada;
	int util1, util2, util_procesadas;
	bool iguales = false, existe = false;
	char procesadas[50]; 
	int apariciones1 = 0, apariciones2 = 0, i, j;
	int cont1 = 0, cont2 = 0, cont_procesadas = 0;
	...
	**/
	...
	if( (una_palabra.Elemento(0) == otra_palabra.Elemento(0)) && (una_palabra.Elemento(util1) == otra_palabra.Elemento(util2 ) ) )
		iguales = true;
	...
	if(iguales){
		...
		for(i=0; i < util1; i++){//Si son similares tendrán la misma longuitud
			...
			while(j < 0){		
				...
				if(una_palabra.Elemento(j) == procesadas[j])
					existe = true;
				else
					existe = false;

				if(!existe){
					procesadas[util_procesadas] = una_palabra.Elemento(j);
					util_procesadas++;
				}
				j--;
		...
		//Cuento aparaciones en cada palabra
		while(cont_procesadas < util_procesadas){
			cont1 = 0;
			cont2 = 0;
			
			//primera palabra
			cont1 = una_palabra.CuentaOcurrencias(procesadas[cont_procesadas]);
			//segunda palabra
			cont2 = otra_palabra.CuentaOcurrencias(procesadas[cont_procesadas]);
